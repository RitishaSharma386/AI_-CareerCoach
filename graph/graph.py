# """
# File: graph.py

# Owner:
# Member 1 – Ritisha Sharma

# Purpose:
# This module defines and compiles the LangGraph workflow for the AI Career Coach project.

# Responsibilities:
# - Creates the StateGraph using AgentState.
# - Registers all workflow nodes.
# - Connects nodes using edges and conditional routing.
# - Invokes individual AI agents.
# - Handles exceptions raised by agents.
# - Compiles the graph into an executable workflow.

# Workflow:

# START
#    │
# Resume
#    │
# Conditional Routing
#  ┌──────────────┬──────────────┬────────────────┐
#  │              │              │
#  ▼              ▼              ▼
# Jobs        Roadmap     Cover Letter
#  │              │              │
#  ▼              ▼              ▼
# (Continue?) (Continue?)       END
#  │
#  ├── Yes → Next Node
#  └── No  → END

# Full Analysis:
# Jobs → Roadmap → Cover Letter → END
# """



# from graph.state import AgentState
# from agent.agent_orchestrator import route
# import agent.agent_resume as agent_resume
# import agent.agent_jobs as agent_jobs
# import agent.agent_roadmap as agent_roadmap
# import agent.agent_cover_letter as agent_cover_letter
# from langgraph.graph import StateGraph, END

# builder = StateGraph(AgentState)

# #Nodes:
# def resume_node(state: AgentState):

#     try:
#         updated_state = agent_resume.run(state)
#         # print("\n===== After Resume Node =====")
#         # print(updated_state)

#         return updated_state

#     except Exception as e:
#         state["error"] = str(e)
#         print("Resume Node Error:", e)
#         return state


# def jobs_node(state: AgentState):

#     try:
#         print("\n========== BEFORE JOBS NODE ==========")
#         print("job_listings:", state["job_listings"])

#         updated_state = agent_jobs.run(state)

#         print("\n========== AFTER JOBS NODE ==========")
#         print("job_listings:", updated_state["job_listings"])
#         print("raw_job_listings:", len(updated_state["raw_job_listings"]))

#         return updated_state

#     except Exception as e:
#         state["error"] = str(e)
#         print("Jobs Node Error:", e)
#         return state


# def roadmap_node(state: AgentState):

#     try:
#         updated_state = agent_roadmap.run(state)
#         # print("\n===== After Roadmap Node =====")
#         # print(updated_state)

#         return updated_state

#     except Exception as e:
#         state["error"] = str(e)
#         print("Roadmap Node Error:", e)
#         return state


# def cover_letter_node(state: AgentState):

#     try:
#         updated_state = agent_cover_letter.run(state)
#         # print("\n===== After Cover Letter Node =====")
#         # print(updated_state)

#         return updated_state

#     except Exception as e:
#         state["error"] = str(e)
#         print("Cover Letter Node Error:", e)
#         return state
    



    
# #Register Nodes:
# builder.add_node("resume",resume_node)
# builder.add_node("jobs",jobs_node)
# builder.add_node("roadmap",roadmap_node)
# builder.add_node("cover_letter",cover_letter_node)

# def after_jobs(state: AgentState):
#     """
#     Determines the next step after the Jobs node.
#     Continue only for full analysis.
#     """
#     if state["user_intent"] == "full_analysis":
#         return "roadmap"
#     return "end"


# def after_roadmap(state: AgentState):
#     """
#     Determines the next step after the Roadmap node.
#     Continue only for full analysis.
#     """
#     if state["user_intent"] == "full_analysis":
#         return "cover_letter"
#     return "end"


# #Connect nodes:
# builder.add_conditional_edges(
#     "resume",
#     route,
#     {
#         "jobs" : "jobs",
#         "roadmap": "roadmap",
#         "cover_letter": "cover_letter",
#         "end": END    
#     }
# )

# builder.add_conditional_edges(
#     "jobs",
#     after_jobs,
#     {
#         "roadmap": "roadmap",
#         "end": END,
#     }
# )

# builder.add_conditional_edges(
#     "roadmap",
#     after_roadmap,
#     {
#         "cover_letter": "cover_letter",
#         "end": END,
#     }
# )

# builder.add_edge("cover_letter", END)

# #Entry point:
# builder.set_entry_point("resume")

# #Compile graph
# graph=builder.compile()






# ------------------------------------
# Dummy State
# ------------------------------------

# dummy_state = {
#     "resume_text": "I know Python",
#     "resume_json": {},
#     "skills": [],
#     "resume_embedding": [],
#     "target_role": "",
#     "raw_job_listings": [],
#     "retrieved_chunks": [],
#     "job_listings": [],
#     "skill_gaps": [],
#     "roadmap": "",
#     "cover_letter": "",
#     "user_intent": "jobs",
#     "error": None
# }
# result = graph.invoke(dummy_state)
# print("\n========== Final State ==========") 
# print(result)

"""
File: graph.py

Owner:
Member 1 – Ritisha Sharma

Purpose:
Defines LangGraph workflow for AI Career Coach.
"""


from graph.state import AgentState

from agent.agent_orchestrator import intent_router

import agent.agent_resume as agent_resume
import agent.agent_jobs as agent_jobs
import agent.agent_roadmap as agent_roadmap
import agent.agent_cover_letter as agent_cover_letter


from langgraph.graph import StateGraph, END



builder = StateGraph(AgentState)



# -------------------------
# Nodes
# -------------------------


def resume_node(state: AgentState):

    try:
        return agent_resume.run(state)

    except Exception as e:
        state["error"] = str(e)
        print("Resume Node Error:", e)
        return state




def jobs_node(state: AgentState):

    try:

        print("\n========== JOB NODE ==========")

        updated_state = agent_jobs.run(state)

        return updated_state


    except Exception as e:

        state["error"] = str(e)
        print("Jobs Node Error:", e)

        return state




def roadmap_node(state: AgentState):

    try:

        print("\n========== ROADMAP NODE ==========")

        return agent_roadmap.run(state)


    except Exception as e:

        state["error"] = str(e)
        print("Roadmap Node Error:", e)

        return state




def cover_letter_node(state: AgentState):

    try:

        print("\n========== COVER LETTER NODE ==========")

        return agent_cover_letter.run(state)


    except Exception as e:

        state["error"] = str(e)
        print("Cover Letter Node Error:", e)

        return state




# -------------------------
# Register Nodes
# -------------------------


builder.add_node(
    "resume",
    resume_node
)


builder.add_node(
    "intent_router",
    intent_router
)


builder.add_node(
    "jobs",
    jobs_node
)


builder.add_node(
    "roadmap",
    roadmap_node
)


builder.add_node(
    "cover_letter",
    cover_letter_node
)




# -------------------------
# Routing after jobs
# -------------------------


def after_jobs(state: AgentState):

    print(
        "After Jobs Intent:",
        state["user_intent"]
    )


    if state["user_intent"] == "full_analysis":

        return "roadmap"


    return "end"





# -------------------------
# Routing after roadmap
# -------------------------


def after_roadmap(state: AgentState):

    print(
        "After Roadmap Intent:",
        state["user_intent"]
    )


    if state["user_intent"] == "full_analysis":

        return "cover_letter"


    return "end"





# -------------------------
# Connect workflow
# -------------------------


builder.add_edge(
    "resume",
    "intent_router"
)



builder.add_conditional_edges(
    "intent_router",
    lambda state: state["user_intent"],
    {

        "find_jobs": "jobs",

        "get_roadmap": "roadmap",

        "write_cover_letter": "cover_letter",

        "full_analysis": "jobs",

        "end": END

    }
)



builder.add_conditional_edges(
    "jobs",
    after_jobs,
    {
        "roadmap": "roadmap",
        "end": END
    }
)



builder.add_conditional_edges(
    "roadmap",
    after_roadmap,
    {
        "cover_letter": "cover_letter",
        "end": END
    }
)



builder.add_edge(
    "cover_letter",
    END
)



# Entry point

builder.set_entry_point(
    "resume"
)



# Compile

graph = builder.compile()